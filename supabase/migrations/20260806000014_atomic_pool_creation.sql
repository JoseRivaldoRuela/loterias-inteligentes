...
create or replace function public.create_betting_pool_with_participants(
    requested_lottery_id uuid,
    requested_game_set_id uuid,
    requested_name text,
    requested_description text,
    requested_status text,
    requested_first_contest integer,
    requested_last_contest integer,
    requested_total_shares integer,
    creator_participates boolean,
    requested_participants jsonb default '[]'::jsonb
)
returns public.betting_pools
language plpgsql
security definer
...
declare
    ...
    created_pool public.betting_pools;
    participant jsonb;
    allocated_shares integer := case when creator_participates then 1 else 0 end;
...
begin
    ...
    if not exists (select 1 from public.saved_game_sets game where game.id = requested_game_set_id and game.owner_id = current_user_id and game.lottery_id = requested_lottery_id and game.betting_pool_id is null) then raise exception 'Jogo indisponível para este bolão.'; end if;
    ...
    for participant in select value from jsonb_array_elements(coalesce(requested_participants, '[]'::jsonb)) loop
        ...
        if position('@' in coalesce(participant ->> 'email', '')) < 2 then raise exception 'Informe um e-mail válido para cada participante.'; end if;
        if length(trim(coalesce(participant ->> 'phone', ''))) < 8 then raise exception 'Informe um telefone válido para cada participante.'; end if;
    end loop;
    ...
    update public.betting_pool_members set share_count = case when creator_participates then 1 else 0 end
    where betting_pool_id = created_pool.id and user_id = current_user_id;

    update public.saved_game_sets set betting_pool_id = created_pool.id where id = requested_game_set_id;

    for participant in select value from jsonb_array_elements(coalesce(requested_participants, '[]'::jsonb)) loop
        participant_email := lower(trim(participant ->> 'email'));
        insert into public.betting_pool_participants (betting_pool_id, name, email, phone, share_count, amount_due)
        values (created_pool.id, trim(participant ->> 'name'), participant_email, trim(participant ->> 'phone'), greatest(1, coalesce((participant ->> 'shareCount')::integer, 1)), 0);
...
grant execute on function public.create_betting_pool_with_participants(uuid, uuid, text, text, text, integer, integer, integer, boolean, jsonb) to authenticated;
