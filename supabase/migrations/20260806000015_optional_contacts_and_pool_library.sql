-- Contatos são opcionais; nome e cotas continuam obrigatórios.
do $$
declare constraint_name text;
begin
  for constraint_name in
    select conname from pg_constraint
    where conrelid = 'public.betting_pool_participants'::regclass
      and contype = 'c' and pg_get_constraintdef(oid) ilike '%phone%'
  loop execute format('alter table public.betting_pool_participants drop constraint %I', constraint_name); end loop;
end $$;
alter table public.betting_pool_participants alter column phone drop not null;
...
declare
 ...
 participant_email text; pool_library_id uuid;
begin
 ...
 update public.saved_game_sets set betting_pool_id = created_pool.id, library_id = pool_library_id where id = requested_game_set_id;
 for participant in select value from jsonb_array_elements(coalesce(requested_participants, '[]'::jsonb)) loop
